...
쿠키는 서버가 어떤 데이터를 브라우저 측에 저장한 후 다시 그 데이터를 받아오는 기술을 말한다. 서버로부터 쿠키를 응답 받은 브라우저는 해당 쿠키를 클라이언트 컴퓨터의 하드디스크에 저장한다. 

```script
Set Cookie : {key:value} // 서버에서 쿠키를 보낼때
Cookie : {key:value} // 브라우저에서 쿠키 보낼때
```
### 쿠키의 속성
